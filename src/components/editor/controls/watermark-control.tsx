...
export function WatermarkControl() {
  ...
  return (
    <div className="relative">
      <div className={`space-y-4 ${!isPro ? "pointer-events-none opacity-40 blur-[2px]" : ""}`}>
        {/* Visibility Toggle */}
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-zinc-500">Show Watermark</span>
        <button
          onClick={() => handleProAction(() => setShowWatermark(!showWatermark))}
      ...
      <div className="relative">
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            value={watermarkText}
            onChange={(e) => {
              if (isPro) setWatermarkText(e.target.value);
            }}
            readOnly={!isPro}
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {(
            ...
            <button
              key={pos.id}
              onClick={() => {
                if (isPro) setWatermarkPosition(pos.id);
      ...
      <div className="relative pt-2 space-y-1">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">Size</span>
          <span className="text-[10px] font-mono text-zinc-400">{watermarkSize}%</span>
        </div>
        <input
          type="range"
          min={50}
          ...
          value={watermarkSize}
          onChange={(e) => {
            if (isPro) setWatermarkSize(Number(e.target.value));
          }}
      ...
      {!isPro && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-2">
          <div className="rounded-xl border border-rose-200/50 bg-white/90 p-4 shadow-xl backdrop-blur-md text-center">
            <h4 className="text-sm font-bold text-rose-900">Custom Watermarks</h4>
            <p className="mt-1 text-xs font-medium text-rose-700/80 leading-relaxed">
              Upgrade to Pro to remove the PrettyShot watermark or customize it with your own brand name.
            </p>
            <Button

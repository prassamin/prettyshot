/**
 * WebKit deviceFrame frame (Safari-style browser window).
 *
 * Unlike the Chromium frame (pure CSS chrome), the WebKit artwork is drawn as
 * an inline SVG overlay: the toolbar gradient, traffic lights, address bar,
 * and tab glyphs are vector paths in the SVG, while the actual web media is
 * rendered as a real DOM element underneath.
 *
 * ── How the composite works ───────────────────────────────────────────────
 * The SVG overlays the whole shell (absolute, z-10) and "punches out" the
 * web viewport region with a `<mask>`: the viewport rect is black in the
 * mask, so the chrome artwork is invisible there and the live media below
 * (z-0) shows through. When no media/children exist the mask is skipped
 * (`hasScreenContent`), so the viewport paints as part of the artwork.
 *
 * The address bar is duplicated twice on purpose:
 *   A static SVG `<text>` + lock glyph group (grays, `mix-blend-luminosity`)
 *      that exports cleanly in DOM-clone screenshots.
 *   A live DOM `<input>` overlay (z-20) that appears only when
 *      `onUrlChange` is wired, letting users edit the URL in the editor.
 *
 * ── Layout model ──────────────────────────────────────────────────────────
 * Shell aspect ratio 1203 × 753; the viewport box is placed at
 * x=1,y=52 with size 1200×700 (percentages below). All artwork uses the
 * design-unit coordinates directly inside the SVG `viewBox`.
 */
...
import type { SafariFrameProps, FrameTone } from "./types";
import type { MediaFit } from "../types";

/** Design-unit geometry of the WebKit window artwork. */
const FRAME_DESIGN_W = 1203;
const FRAME_DESIGN_H = 753;

/** Where the web viewport sits inside the artwork, in design units. */
const VIEWPORT_X = -1;
const VIEWPORT_Y = 51;
const VIEWPORT_W = 1204;
const VIEWPORT_H = 703;
...
/** Percentage placement of the live address input overlay. */
const ADDRESS_LEFT_PCT = (286 / FRAME_DESIGN_W) * 100;
const ADDRESS_TOP_PCT = (11 / FRAME_DESIGN_H) * 100;
const ADDRESS_WIDTH_PCT = (666 / FRAME_DESIGN_W) * 100;
const ADDRESS_HEIGHT_PCT = (30 / FRAME_DESIGN_H) * 100;

/** The rounded viewport opening cut out of the chrome artwork. */
const VIEWPORT_OPENING_PATH =
  "M0 52H1202V741C1202 747.627 1196.63 753 1190 753H12C5.37258 753 0 747.627 0 741V52Z";

/**
 * A static, theme-aware Safari-style browser deviceFrame.
 *
 * @see SafariFrameProps for the full prop contract.
 */
export function SafariFrame({
  ...
  mediaCss,
  fit = "cover",
  viewportRadius = "0 0 11px 11px",
  urlValue,
  urlPlaceholder = "Search or enter website name",
  onUrlChange,
  shimmer = true,
  className,
  style,
  ...
  // Mask/clip ids must be unique per mounted instance (the picker grid
  // renders many at once) — `useId` guarantees that.
  const rawId = useId().replace(/:/g, "");
  const punchMaskId = `webkit-punch-${rawId}`;
  const artworkClipId = `webkit-artwork-${rawId}`;
  const roundedCutId = `webkit-rounded-${rawId}`;

  const hasMedia = Boolean(mediaSrc);
  const hasScreenContent = hasMedia || Boolean(children);

  const addressText = urlValue ?? url ?? "";
  const editable = Boolean(onUrlChange);

  const chromeFill =
    tone === "dark" ? "#404040" : tone === "light" ? "#E5E5E5" : undefined;
  const toolbarFill =
    tone === "dark" ? "#262626" : tone === "light" ? "#ffffff" : undefined;
  // Theme-following fallbacks (Tailwind dark: variants).
  const chromeFillClass = chromeFill ? undefined : "fill-[#404040]";
  const toolbarFillClass = toolbarFill ? undefined : "fill-[#262626]";

  const viewportBg = viewportBgFor(tone);

  const viewportBoxStyle: CSSProperties = {
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* Live media layer — underneath the artwork. */}
      {mediaSrc ? (
        <div
          ref={viewportRef}
          className={`pointer-events-none absolute z-0 overflow-hidden ${viewportBg.className}`}
      ...
      {/* Live editable address input (editor only, z-20 over the artwork).
          Swallows pointer events so stage dragging never starts from it. */}
      {editable ? (
        <label
        ...
        >
          <span className="flex max-w-full items-center justify-center gap-[0.45cqw]">
            ...
            <div className="grid max-w-[52ch] min-w-[9ch] items-center">
              <span
                aria-hidden
                className="col-start-1 row-start-1 invisible whitespace-pre p-0 m-0 leading-none font-sans text-[clamp(7px,1cqw,12px)]"
              >
                {addressText || urlPlaceholder}
              </span>
              <input
                type="text"
                inputMode="url"
                ...
                className="col-start-1 row-start-1 w-full border-0 bg-transparent p-0 m-0 leading-none text-left font-sans text-[clamp(7px,1cqw,12px)] text-[#8a8a8a] outline-none placeholder:text-[#a3a3a3]"
              />
            </div>
          </span>
        </label>
      ...
      {/* Vector chrome artwork — punches the viewport opening via mask. */}
      <svg
        viewBox={`0 0 ${FRAME_DESIGN_W} ${FRAME_DESIGN_H}`}
        fill="none"
      ...
      >
        <defs>
          {/* The mask "punch": white = keep artwork, black = show media. */}
          <mask id={punchMaskId} maskUnits="userSpaceOnUse">
            <rect
              x="0"
              y="0"
              width={FRAME_DESIGN_W}
              height={FRAME_DESIGN_H}
              fill="white"
            />
            <path d={VIEWPORT_OPENING_PATH} fill="black" />
          </mask>

          <clipPath id={artworkClipId}>
            <rect width={FRAME_DESIGN_W} height={FRAME_DESIGN_H} fill="white" />
          </clipPath>

          <clipPath id={roundedCutId}>
            <path d={VIEWPORT_OPENING_PATH} fill="white" />
          </clipPath>
        </defs>

        <g
          clipPath={`url(#${artworkClipId})`}
          mask={hasScreenContent ? `url(#${punchMaskId})` : undefined}
        >
          {/* Chrome body + rounded top bar. */}
          <path
            d={VIEWPORT_OPENING_PATH}
            fill={chromeFill}
          ...
          {/* Lock glyph + URL text — hidden when a live input is on top. */}
          <g className="mix-blend-luminosity" opacity={editable ? 0 : 1}>
            <path
          ...
          <g className="mix-blend-luminosity">
            ...
            >
              {editable ? "" : addressText}
            </text>
          </g>

          {/* Toolbar glyphs (default density only). */}
          {mode === "default" ? (
            <>
              <g className="mix-blend-luminosity">
                <path
...
/**
 * Viewport backdrop for a given tone.
 *
 * Dark mode uses the toolbar color behind `contain` media; light mode uses
 * white. Returns both the Tailwind class and an inline-style twin (the
 * inline styles survive the static export/DOM-clone pipeline).
 */
function viewportBgFor(tone: FrameTone): {
  className: string;
  ...
  if (tone === "dark")
    return {
      className: "bg-[#262626]",
      inline: { backgroundColor: "#262626" },
    };
  if (tone === "light")
    return { className: "bg-white", inline: { backgroundColor: "#ffffff" } };
  return { className: "bg-white dark:bg-[#262626]", inline: {} };
}
